...
---------------------------Menu-----------------------------
------------------------------------------------------------
Cfg, menu = uiconfig.add_menu('Swain', 250)
...
------------------------------------------------------------
function UseQ(targ)
	if GetDistance(targ, myHero) <= 625 then CastSpellTarget('Q', targ) end
end

function UseW(targ)
	CastPosition,  HitChance,  Position = YP:GetCircularCastPosition(targ, .5, 265, 900, 1250, myHero, false)
...
function UseE(targ)
	if GetDistance(targ, myHero) <= 625 then CastSpellTarget('E', targ) end
end

function UseR(targ)
	if GetDistance(targ, myHero) <= 690 and IsBuffed(myHero, 'swain_demonForm_idle.troy') ~= true then CastSpellTarget('R', myHero) end
end
...
function SpellClear()
	...
	--R Farm
	if Cfg['1. Skill Options'].R_LC_ON and minionTarget ~= nil and IsBuffed(myHero, 'swain_demonForm_idle.troy') ~= true then CastSpellTarget('R', myHero) end
end
------------------------------------------------------------
------------------------End Of Skills-----------------------
------------------------------------------------------------
...
function RangeIndicators()
	...
	if Cfg['3. Draw Range'].qRange then
		DrawCircleObject(myHero, 625, Cfg['3. Draw Range'].qRangeColor)
	end
	if Cfg['3. Draw Range'].wRange then
		DrawCircleObject(myHero, 900, Cfg['3. Draw Range'].wRangeColor)
	end
	if Cfg['3. Draw Range'].eRange then
		DrawCircleObject(myHero, 626, Cfg['3. Draw Range'].eRangeColor)
	end
	if Cfg['3. Draw Range'].rRange then
		DrawCircleObject(myHero, 700, Cfg['3. Draw Range'].rRangeColor)
	end
end
------------------------------------------------------------
...
-- * lbc_champion = Swain // if this script is for a special champ
-- * lbc_tags = Swain, Bird, birdman, AP, Kool, kool, iskool, koolkaracter
-- * lbc_link = http://leaguebot.net/forum/Upload/showthread.php?tid=4288
-- * lbc_source = https://raw.githubusercontent.com/koolkaracter/Scripts/AutoKool/Champs/SwainIsKool.lua
