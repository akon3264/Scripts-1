-- ************************** LBC META *****************************
-- * lbc_name = ShacoIsKool.lua
-- * lbc_version = 1.0
-- * lbc_date = 09/28/2014 // use correct date format mm/dd/yyyy
...
local skillOrder = {}
local qRange, wRange, eRange, rRange = 400, 425, 625, 2200       						
...
---------------------------Menu-----------------------------
------------------------------------------------------------
Cfg, menu = uiconfig.add_menu('Shaco Is Kool', 250)
...
submenu.keytoggle('E_Auto_ON', 'Auto Harrass w/ E', Keys.Z, false)
submenu.permashow('E_Auto_ON')
submenu.keydown('Control_Ult', 'Clone Attack Target Key', Keys.R)
submenu.checkbox('Q_Danger_ON', 'Q at x% Health', false)
submenu.slider('Q_Danger_ON_Value', 'Q at % Health Value', 0, 100, 25)
submenu.keydown('Set_Home', 'Set Home Key for Q', Keys.H)
...
menu.label('lb01', ' ')
menu.label('lb02', 'ShacoIsKool Version '..tostring(Version) ..' by KoolKaracter')
------------------------------------------------------------  
------------------------End Of Menu-------------------------
------------------------------------------------------------
...
function Main()
	...
	AutoPots()
	onTick()
	if Cfg['1. Skill Options'].E_Auto_ON and target ~= nil then UseE(target) end
	if (target ~= nil or ValidTarget(targetPri, rRange) or ultTarget ~= nil) and Cfg['1. Skill Options'].Control_Ult and myHero.SpellNameR == 'hallucinateguide' then 
		if target ~= nil then 
			CastSpellTarget('R', target)
		elseif targetPri ~= nil and GetDistance(targetPri, myHero) < rRange then 
			CastSpellTarget('R', targetPri)
		elseif target == nil and (targetPri == nil or GetDistance(targetPri, myHero) >= rRange) and ultTarget ~= nil then 
			CastSpellTarget('R', ultTarget)
		end
	end
	if Cfg['1. Skill Options'].Q_Danger_ON then 
		DrawIt()
		QIfLow() 
	end
	if Cfg['1. Skill Options'].Set_Home then SetHome() end	
	...
	if yayo.Config.AutoCarry then 
		...
		if target == nil then 
			if myHero.SpellNameR == 'hallucinateguide' and Cfg['1. Skill Options'].R_AC_ON and ultTarget ~= nil then UseR(ultTarget) end
		end
	end                      
	...
	if yayo.Config.Mixed then 
		...
		if target == nil then 
			if myHero.SpellNameR == 'hallucinateguide' and Cfg['1. Skill Options'].R_M_ON and ultTarget ~= nil then UseR(ultTarget) end
		end
	end
...
---------------------------Skills---------------------------						
------------------------------------------------------------
local homeX = myHero.x
local homeY = myHero.y
local homeZ = myHero.z

function SetHome()
	homeX = GetCursorWorldX()
	homeY = GetCursorWorldY()
	homeZ = GetCursorWorldZ()
end

function QIfLow()
	if myHero.health < myHero.maxHealth*(Cfg['1. Skill Options'].Q_Danger_ON_Value / 100) and CanUseSpell('Q') and target ~= nil then UseQ() end
end
                              
function DrawIt()
	CustomCircle(0,50,0, "", homeX, homeY, homeZ)
	DrawTextMinimap('H', homeX, homeZ, Color.Yellow)
end

function UseQ()
	if IsBuffed(myHero, "FountainHeal") ~= true and IsBuffed(myHero, 'TeleportHome.troy') ~= true then CastSpellXYZ('Q',homeX,homeY,homeZ) end
end

function UseW(targ)
    if GetDistance(targ, myHero) <= wRange then CastSpellXYZ('W', targ.x, targ.y, targ.z ) end
end
...
function UseR(targ)
	if  GetDistance(targ, myHero) <= 400 and ValidTarget(targ) and myHero.SpellNameR == 'HallucinateFull' then  --initial cast
...
function SpellLastHit()
	...
	if minionTarget ~= nil and GetDistance(minionTarget, myHero) > 190 then 
		--E Farm
		if Cfg['1. Skill Options'].E_LH_ON and minionTarget ~= nil and minionTarget.dead ~= 1 and getDmg('E', minionTarget, myHero) >= minionTarget.health then 
...
--UltTarget Info
	if (ultTarget ~= nil and (ultTarget.dead == 1 or myHero.dead == 1)) or (ultTarget ~= nil and GetDistance(ultTarget, myHero) > rRange) then ultTarget = nil end
	if targetPri ~= nil and GetDistance(targetPri, myHero) < rRange then 
		ultTarget = targetPri
	else 
		ultTarget = GetWeakEnemy(TSType, rRange)
		if ultTarget ~= nil then 
			CustomCircle(100,10,4,ultTarget)
		end
	end
end
------------------------------------------------------------
