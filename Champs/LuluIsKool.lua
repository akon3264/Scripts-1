-- ************************** LBC META *****************************
-- * lbc_name = LuluIsKool.lua
-- * lbc_version = 1.0
-- * lbc_date = 09/30/2014 // use correct date format mm/dd/yyyy
...
local skillOrder = {}
local qRange, wRange, eRange, rRange = 900, 650, 650, 850      					
local qSpeed, wSpeed, eSpeed, rSpeed = 1300, nil, nil, nil    					
local qDelay, wDelay, eDelay, rDelay = .5, .5, .5, .625          					
local qWidth, wWidth, eWidth, rWidth = 60, nil, nil, nil								
...
---------------------------Menu-----------------------------
------------------------------------------------------------
Cfg, menu = uiconfig.add_menu('Lulu Is Kool', 250)
local submenu = menu.submenu('1. Skill Options', 400)
...
submenu.checkbox('Locket', 'Auto Use Locket of Solari', true)
submenu.slider('LocketValue', 'Use Locket at X% health', 0, 100, 20)
submenu.checkbox('FOM', 'Auto Use Face of the Mountain', true)
submenu.slider('FOMValue', 'Use Face of the Mountain at X% health', 0, 100, 20)
submenu.checkbutton('Require_Targ_For_D_I_ON', 'Require Targ near Ally for LoIS and FotM', true)
...
submenu.slider('Health_Potion_Value', 'Health Potion Value', 0, 100, 35)
submenu.checkbox('Biscuit_ON', 'Biscuit', true)
submenu.slider('Biscuit_Value', 'Biscuit Value', 0, 100, 35)
submenu.checkbox('Chrystalline_Flask_ON', 'Chrystalline Flask', true)
submenu.slider('Chrystalline_Flask_Value', 'Chrystalline Flask Value', 0, 100,35)
submenu.checkbox('Elixir_of_Fortitude_ON', 'Elixir of Fortitude', true)
submenu.slider('Elixir_of_Fortitude_Value', 'Elixir of Fortitude Value', 0, 100, 35)
submenu.checkbox('Mana_Potion_ON', 'Mana Potions', true)
submenu.slider('Mana_Potion_Value', 'Mana Potion Value', 0, 100, 35)
...
submenu.checkbox('KSEQ', 'KS with E/Q', true) --Will cast E to minion or enemy champ and and then cast Q to ks... this allows for longer range. 
submenu.checkbox('KSDFG', 'KS with DFG', true) -- Deathfire Grasp
submenu.checkbox('KSBFT', 'KS with BFT', true) -- Blackfire Torch
submenu.checkbox('KSFQC', 'KS with FQC', true) -- Frost queens claim
...
menu.label('lb01', ' ')
menu.label('lb02', 'Lulu Is Kool Version '..tostring(Version) ..' by KoolKaracter')
...
function Main()
	...
	AutoPots()
	GetAlly()
	if Cfg['1. Skill Options'].Mode_Switch then
		times = 1
	elseif times == 1 and not Cfg['1. Skill Options'].Mode_Switch then 
		if Cfg['1. Skill Options'].Mode_Selector == 1 then 
			Cfg['1. Skill Options'].Mode_Selector = 2
			times = 0
		else
			Cfg['1. Skill Options'].Mode_Selector = 1
			times = 0
		end	
	end 
	if pixy ~= nil and Cfg['1. Skill Options'].Pixy_Sphere_ON then DrawSphere(30,30,0,pixy.x,pixy.y,pixy.z) end --Make this an option
--	if Cfg['1. Skill Options'].EQ_ON and target ~= nil then UseEQ(target) end
	if Cfg['1. Skill Options'].Auto_Ult_ON then UseR() end
...
------------------------------------------------------------
function UseQ(targ)
	if GetDistance(targ, myHero) <= qRange and ValidTarget(targ) and IsSpellReady('Q') then  
			CastPosition,  HitChance,  Position = YP:GetLineAOECastPosition(targ, qDelay, qWidth, qRange, qSpeed, myHero)
			if CastPosition and HitChance >= 2 then 
				local x, y, z = CastPosition.x, CastPosition.y, CastPosition.z                
				CastSpellXYZ('Q', x, y, z)
			end
	elseif pixy ~= nil and GetDistance(targ, pixy) <= qRange and ValidTarget(targ) and IsSpellReady('Q') then  
			CastPosition,  HitChance,  Position = YP:GetLineAOECastPosition(targ, qDelay, qWidth, qRange, qSpeed, pixy)
			if CastPosition and HitChance >= 2 then 
				local x, y, z = CastPosition.x, CastPosition.y, CastPosition.z                
				CastSpellXYZ('Q', x, y, z)
			end
...
function UseW(targ)
	if GetDistance(targ, myHero) <= wRange and ValidTarget(targ) and IsSpellReady('W') then CastSpellTarget('W', targ) end
end


function UseE(targ)
	if targ ~= nil and Cfg['1. Skill Options'].Mode_Selector == 2 and IsSpellReady('E') then 
...
function UseEQ(targ)
	...
	for i = 1, objManager:GetMaxCreatures() do
		local creature = objManager:GetCreature(i)
		if GetDistance(targ, myHero) > qRange and GetDistance(creature, myHero) < eRange and GetDistance(creature, targ) < (qRange - 25) and  creature ~= myHero then
			eTarg =  creature
		else
			--do nothing
		end
	...
	if eTarg ~= nil then 	
		if eTarg.team ~= myHero.team then 
			if getDmg('E', eTarg, myHero) < eTarg.health then 
			print(eTarg.name)
			UseE(eTarg)
			end
		else
	...
	if GetDistance(pixy, targ) < qRange or GetDistance(myHero, targ) < qRange then 
		UseQ(targ)
	end      
end
...
function UseR()
	...
	local ally = objManager:GetHero(i)
		if ally ~= nil and ally.team == myHero.team and ally.visible == 1 and GetDistance(myHero, ally) < rRange then 
			if ally.health <= (ally.maxHealth*(Cfg['1. Skill Options'].Auto_Ult_Value / 100)) and IsBuffed(ally, "FountainHeal") ~= true and IsBuffed(ally, 'TeleportHome.troy') ~= true  and IsBuffed(ally, 'TeleportHomeImproved.troy') ~= true and EnemyNear(ally) then 
...
function SpellClear()
	...
	--Q Farm
	if Cfg['1. Skill Options'].Q_LC_ON and minionTarget ~= nil and IsSpellReady('Q') then UseQ(minionTarget) end
	--E Farm
	if Cfg['1. Skill Options'].E_LC_ON and minionTarget ~= nil and IsSpellReady('E') then UseE(minionTarget) end
end

---Auto Shield Functions
function autoE(ally)
	if ally ~= nil and ally.health <= (ally.maxHealth*(Cfg['1. Skill Options'].Auto_E_Value / 100)) and IsSpellReady('E') and Cfg['1. Skill Options'].Auto_E_ON then
...
function UseOffensiveItems(target)
    ...
    if target ~= nil then
			...
			if Cfg['4. Item Options'].FQC and (GetDistance(myHero, target) < 850) then -- IR
...
function UseDefensiveItems()
	if IsBuffed(myHero, "FountainHeal") ~= true then
		for i = 1, objManager:GetMaxHeroes() do
		local ally = objManager:GetHero(i)
			if Cfg['4. Item Options'].Require_Targ_For_D_I_ON and ally ~= nil and ally.team == myHero.team and ally.visible == 1 and GetDistance(myHero, ally) < 600 then
				if target ~= nil and Cfg['4. Item Options'].Locket and ally.health <= (ally.maxHealth*(Cfg['4. Item Options'].LocketValue / 100)) and GetDistance(ally, target) < 900 then --If health is below the slider % value
					UseItemOnTarget(3190,ally) -- Locket of Iron Solari
				end
				if target ~= nil and Cfg['4. Item Options'].FOM and ally.health <= (ally.maxHealth*(Cfg['4. Item Options'].FOMValue / 100)) and GetDistance(ally, target) < 900 then --If health is below the slider % value
					UseItemOnTarget(3401,ally) -- Face of the Mountain
				end

			elseif Cfg['4. Item Options'].Require_Targ_For_D_I_ON ~= true and ally ~= nil and ally.team == myHero.team and ally.visible == 1 and GetDistance(myHero, ally) < 600 and IsBuffed(ally, 'TeleportHome.troy') ~= true  and IsBuffed(ally, 'TeleportHomeImproved.troy') ~= true then
			
				if CfgKoolSettings['1. Kool Offensive Items'].Locket and ally.health <= (ally.maxHealth*(Cfg['4. Item Options'].LocketValue / 100)) then --If health is below the slider % value
					UseItemOnTarget(3190,ally) -- Locket of Iron Solari
				end
				if CfgKoolSettings['1. Kool Offensive Items'].FOM and ally.health <= (ally.maxHealth*(Cfg['4. Item Options'].FOMValue / 100)) then --If health is below the slider % value
					UseItemOnTarget(3401,ally) -- Face of the Mountain
				end
			end
		end		
...
function KillSteal()
	...
	for i = 1, objManager:GetMaxHeroes() do
          local ksTarg = objManager:GetHero(i)
			...
			if Cfg['7. Kill Steal Options'].KSEQ and ksTarg ~= nil and ksTarg.team ~= myHero.team and ValidTarget(ksTarg) and getDmg('Q', ksTarg, myHero) >= ksTarg.health and IsSpellReady('Q') and IsSpellReady('E') then UseEQ(ksTarg) end
...
function ShowPercentHP()
       ...
       --[[ local myHP = ((myHero.health / myHero.maxHealth) * 100)
		...
		for i = 1, objManager:GetMaxHeroes() do
			local ally = objManager:GetHero(i)
			if ally.dead ~= 1 and ally.team == myHero.team and ally.visible == 1 then 
				local allyHP = ((ally.health /ally.maxHealth) * 100)
				
				allyHP = string.format('%d%%', allyHP)
				DrawTextObject(allyHP,ally,Color.White)
			end
		end
...
function FindPixy()
--Pixy is RobotBuddy/LuluFaerie
	for i = 1, objManager:GetMaxObjects() do
        local obj = objManager:GetObject(i)
        if obj ~= nil  then
			if obj.name == 'LuluFaerie' or obj.charName == 'LuluFaerie' then 
				pixy = obj
			end
		end
	end	
...
function EnemyNear(ally)
	i = 1
	maxheros = objManager:GetMaxHeroes()
	enemynear = false
	repeat
		Champ = objManager:GetHero(i)
		if Champ.team ~= myHero.team and Champ.dead ~= 1 and GetDistance(Champ, ally) < 900 then 
			enemynear = true
		end
		i = i + 1
	until (enemynear == true or i > maxheros)
	
	if enemynear == true then 
		return true
	else 
		return false
	end
