...
( \     (  ____ (  ___ \( \     (  ___  | (    /(  ____ \  \__   __(  ____ \  | \    /(  ___  |  ___  | \      
| (     | (    \/ (   ) ) (     | (   ) |  \  ( | (    \/     ) (  | (    \/  |  \  / / (   ) | (   ) | (      
| |     | (__   | (__/ /| |     | (___) |   \ | | |           | |  | (_____   |  (_/ /| |   | | |   | | |      
| |     |  __)  |  __ ( | |     |  ___  | (\ \) | |           | |  (_____  )  |   _ ( | |   | | |   | | |      
| |     | (     | (  \ \| |     | (   ) | | \   | |           | |        ) |  |  ( \ \| |   | | |   | | |      
| (____/\ (____/\ )___) ) (____/\ )   ( | )  \  | (____/\  ___) (__/\____) |  |  /  \ \ (___) | (___) | (____/\
(_______(_______// \___/(_______//     \|/    )_|_______/  \_______|_______)  |_/    \(_______|_______|_______/
...
local skillOrder = {}
local qRange, wRange, eRange, rRange = 700, 700, 950, 700     						
local qSpeed, wSpeed, eSpeed, rSpeed = nil, nil, 1600, nil     						
local qDelay, wDelay, eDelay, rDelay = .5, .5, .25, .50          					
local qWidth, wWidth, eWidth, rWidth = nil, 250, 95, nil								
...
---------------------------Menu-----------------------------
------------------------------------------------------------
Cfg, menu = uiconfig.add_menu('Leblanc Is Kool', 250)
...
submenu.checkbox('Ult_AC_ON', 'Force Mimic Q', true)
...
submenu.checkbox('Ult_M_ON', 'Force Mimic Q', true)
...
submenu.checkbox('Ult_Q_ON', 'Mimic Q', true)
submenu.checkbox('Ult_W_ON', 'Mimic W', false)
submenu.checkbox('Ult_E_ON', 'Mimic E', false)
...
submenu.label('lbS1', 'Ult Range Not Supported')
...
menu.label('lb01', ' ')
menu.label('lb02', 'Leblanc Is Kool Version '..tostring(Version) ..' by KoolKaracter')
------------------------------------------------------------  
------------------------End Of Menu-------------------------
------------------------------------------------------------
...
function Main()
	...
	if yayo.Config.AutoCarry then 
		if target ~= nil then 
			...
			if Cfg['1. Skill Options'].Ult_AC_ON and myHero.SpellLevelQ >= 1 and myHero.SpellLevelR >= 1 and Cfg['1. Skill Options'].Q_AC_ON and Cfg['1. Skill Options'].R_AC_ON then
				UseQRCombo(target)
	...
	if yayo.Config.Mixed then 
		if target ~= nil then 
			...
			if Cfg['1. Skill Options'].Ult_M_ON and myHero.SpellLevelQ >= 1 and myHero.SpellLevelR >= 1 and Cfg['1. Skill Options'].Q_M_ON and Cfg['1. Skill Options'].R_M_ON then
				UseQRCombo(target)
				if Cfg['1. Skill Options'].W_M_ON then UseW(target) end
				if Cfg['1. Skill Options'].E_M_ON then UseE(target) end
				if Cfg['1. Skill Options'].W_M_ON then UseW2(target) end
			else
...
------------------------------------------------------------
function UseQ(targ)
	if GetDistance(targ, myHero) <= qRange and myHero.SpellTimeQ > 1.0 then 
		CastSpellTarget('Q', targ)
	end
end

function UseW(targ)
	if GetDistance(targ, myHero) <= wRange and myHero.SpellNameW ~= 'leblancslidereturn' and myHero.SpellTimeW > 1.0 then 
...
function UseR(targ)
	if myHero.SpellTimeR >= 1.0 then
		...
		if Cfg['1. Skill Options'].Ult_W_ON and myHero.SpellNameR == 'LeblancSlideM' and GetDistance(targ, myHero) <= wRange and myHero.SpellNameR ~= 'leblancslidereturn' then 
			CastSpellTarget('R', targ)
		end
		if Cfg['1. Skill Options'].Ult_E_ON and myHero.SpellNameR == 'LeblancSoulShackleM' and GetDistance(targ, myHero) <= eRange then 
			CastPosition,  HitChance,  Position = YP:GetLineCastPosition(targ, eDelay, eWidth, eRange, eSpeed, myHero, eCollision)
			if CastPosition and HitChance >= 2 then 
				local x, y, z = CastPosition.x, CastPosition.y, CastPosition.z                
...
function SpellClear()
	local minionTarget = GetLowestHealthEnemyMinion(700)
	if Cfg['1. Skill Options'].Q_LC_ON and minionTarget ~= nil and minionTarget.dead ~= 1 and myHero.SpellTimeQ > 1.0 then UseQ(minionTarget) end
	if Cfg['1. Skill Options'].W_LC_ON and minionTarget ~= nil and minionTarget.dead ~= 1 and myHero.SpellTimeW > 1.0 then UseW(minionTarget) end
	if Cfg['1. Skill Options'].R_LC_ON and minionTarget ~= nil and minionTarget.dead ~= 1 and myHero.SpellTimeR > 1.0 then UseR(minionTarget) end
	if Cfg['1. Skill Options'].E_LC_ON and minionTarget ~= nil and minionTarget.dead ~= 1 and myHero.SpellTimeE > 1.0 then UseE(minionTarget) end
