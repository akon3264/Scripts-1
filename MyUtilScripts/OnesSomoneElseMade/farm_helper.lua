...
local SORT_CUSTOM = function(a, b) return a.maxHealth and b.maxHealth and a.maxHealth < b.maxHealth end
	...
	lasthitCFG, menu = uiconfig.add_menu('Last Hit Config', 200)
...
function Main()
	if IsLolActive() then
		...
		for i, minionAA in pairs(MinionsAA) do
			...
			if lasthitCFG.AA and AAdam~=nil and minionAA.health<AAdam then CustomCircleXYZ(lasthitCFG.AAradius,lasthitCFG.AAthickness,lasthitCFG.AAcolor,minionAA) end
		end
		for i, minionQ in pairs(MinionsQ) do
			if minionQ~=nil then Qdam = getDmg("Q",minionQ,myHero) end
			if lasthitCFG.Q and Qdam~=nil and minionQ.health<Qdam then CustomCircleXYZ(lasthitCFG.Qradius,lasthitCFG.Qthickness,lasthitCFG.Qcolor,minionQ) end
		end
		for i, minionW in pairs(MinionsW) do
			if minionW~=nil then Wdam = getDmg("W",minionW,myHero) end
			if lasthitCFG.W and Wdam~=nil and minionW.health<Wdam then CustomCircleXYZ(lasthitCFG.Wradius,lasthitCFG.Wthickness,lasthitCFG.Wcolor,minionW) end
		end
		for i, minionE in pairs(MinionsE) do
			if minionE~=nil then Edam = getDmg("E",minionE,myHero) end
			if lasthitCFG.E and Edam~=nil and minionE.health<Edam then CustomCircleXYZ(lasthitCFG.Eradius,lasthitCFG.Ethickness,lasthitCFG.Ecolor,minionE) end
		end
		for i, minionR in pairs(MinionsR) do
			if minionR~=nil then Rdam = getDmg("R",minionR,myHero) end
			if lasthitCFG.R and Rdam~=nil and minionR.health<Rdam then CustomCircleXYZ(lasthitCFG.Rradius,lasthitCFG.Rthickness,lasthitCFG.Rcolor,minionR) end
		end
	end
end
